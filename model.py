...
def positionwise_FF(inp, d_model, d_inner, dropout, kernel_initializer,
    ...
    with tf.variable_scope(scope):
        ...
        output = keras.layers.LayerNormalization(axis=-1)(output+inp + inp)
        # print(output,"-------------------------------------------------------------")
        # output = keras.layers.LayerNormalization(axis=-1)
        # output = output(output+inp + inp)
    return output
...
def rel_multihead_attn(w, r, r_w_bias, r_r_bias, attn_mask, mems, d_model,
    ...
    with tf.variable_scope(scope):
        ...
        # output = tf.python.keras.layers.normalization.LayerNormalization(axis=-1)(attn_out + w)
        # print(type(output),"-------------------------------------")
        output = keras.layers.LayerNormalization(axis=-1)(attn_out+w)
        # output = output
    return output
